...
class ForecastExp(BaseRelevant, BaseIrrelevant, ForecastSettings):
    ...
    def _init_optimizer(self):
        if hasattr(self.model, "beta_scheduler") and (self.model.beta_scheduler is not None):
            beta_params = list(self.model.beta_scheduler.parameters())
            ...
            self.model_optim = parse_type(self.optm_type, globals=globals())(
                other_params, lr=self.lr, weight_decay=self.l2_weight_decay
            )

            self.model_optim_beta = parse_type(self.optm_type, globals=globals())(
                beta_params, lr=self.lr, weight_decay=self.l2_weight_decay
            )
        else:
            self.model_optim = parse_type(self.optm_type, globals=globals())(
                self.model.parameters(), lr=self.lr, weight_decay=self.l2_weight_decay
            )
            self.model_optim_beta = None
